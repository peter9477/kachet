...
export const api = {
  ...
  saveSettings: (patch) => req('/api/settings', json('PUT', patch)),
  report: (kind, params) => req(`/api/reports/${kind}?` + new URLSearchParams(params)),
  fetchBocRates: (end) => req('/api/prices/fetch-boc', json('POST', { end })),
  reportConfigs: () => req('/api/report-configs'),
  createReportConfig: (c) => req('/api/report-configs', json('POST', c)),
  updateReportConfig: (id, c) => req(`/api/report-configs/${id}`, json('PUT', c)),
  deleteReportConfig: (id) => req(`/api/report-configs/${id}`, { method: 'DELETE' }),
